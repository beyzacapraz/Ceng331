...
# FETCH PART
# JTAB contains value C and register rb, the rest 8 bytes are constant word.
# Because of that I added IJTAB to need_regids, instr_valid and need_valC.
# I added "W_icode == IJTAB : W_valM" to f_pc because it will behave like return.
# I did not add JTAB to f_predPC because we are calculating the jump address on our own.

# DECODE PART
# For read port addresses D_srcA and D_srcB, because we have rb I updated
# D_srcB part. Since we do not need to write back the result of ALU 
# I did not update the dest parts.

# EXECUTE PART
# I set the A input of ALU to E_valC and B input to E_ValB
# The instruction does not set any condition codes.

# MEMORY PART
# We should read the calculated address ValE from the memory.
# Because of that I added JTAB to mem_read and med_address M_ValE.

# There is no PC UPDATE PART, in pipeline demonstration we do not have PC register.

# PIPELINE REGISTER CONTROL PART
# Since JTAB behaves like  return, it will perform stall in fetch, bubble
# in decode, and the rest will be normal behaviour. Because of that I added
# "IJTAB in  { D_icode, E_icode, M_icode }" in F_stall and D_bubble part.
...
wordsig f_valP	'if_id_next->valp'   # AW_icode == IJTAB : W_valM; #??ddress of following instruction
